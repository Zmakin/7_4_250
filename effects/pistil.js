// Three-native Pistil shell (2-color) — standalone, inscribed on its own.
// FRAME-BASED contract (see effects/peony.js).
//
// Pistil = a colored outer peony with a SMALL contrasting core ("pistil") in the
// middle. Two populations in one Points cloud: a wide outer shell (opts.color)
// and a tight inner sphere (opts.color2) thrown shorter so it sits inside.
import {
...
export const FPS = 60;
export const FRAMES = 132;
export const DURATION = Math.round(FRAMES / FPS * 1000); // 2200
...
export function create(opts = {}) {
  const outer = new Color(opts.color  || '#ff3b6b');             // shell
  const inner = new Color(opts.color2 || '#ffe14d');            // pistil core
  ...
  const drag    = opts.drag    != null ? opts.drag    : 1.05;
  const spread  = (opts.spread || 16) * (opts.scale || 1);
  const coreFrac= opts.coreFrac!= null ? opts.coreFrac: 0.42;    // share of stars in the core
  const r0      = 0.55;

  const pos  = new Float32Array(count * 3);
  const col  = new Float32Array(count * 3);
  const cBase= new Float32Array(count * 3);  // per-particle base color
  const v0   = new Float32Array(count * 3);
  const base = new Float32Array(count);
  const seed = new Float32Array(count);

  for (let i = 0; i < count; i++) {
    const isCore = Math.random() < coreFrac;
    const sp = isCore ? spread * 0.45 : spread;                  // core thrown shorter -> sits inside
    const theta = Math.random() * Math.PI * 2;
    const phi   = Math.acos(2 * Math.random() - 1);
    const speed = sp * (0.6 + Math.random() * 0.4);
    v0[i*3]   = Math.sin(phi) * Math.cos(theta) * speed;
    v0[i*3+1] = Math.cos(phi) * speed;
    v0[i*3+2] = Math.sin(phi) * Math.sin(theta) * speed;
    base[i]   = r0 / speed;
    const c = isCore ? inner : outer;
  ...
  function setAgeSeconds(t) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const tw = 0.85 + 0.15 * Math.sin(t * 14 + seed[i] * 40);
      const bright = fade * tw;
      col[ix] = cBase[ix] * bright; col[ix + 1] = cBase[ix + 1] * bright; col[ix + 2] = cBase[ix + 2] * bright;
    }

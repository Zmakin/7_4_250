...
export const PEAK_FRAME = 73;       // spacebar option 2 = peak ~1.20 s in ((73-1)/60)
...
export function create(opts = {}) {
  ...
  const streams  = opts.count   || 130;                          // number of streams
  const trailLen = opts.trail   || 11;                           // points per stream
  ...
  const gravity  = opts.gravity != null ? opts.gravity : 12.0;   // weep dominates
  const drag     = opts.drag    != null ? opts.drag    : 1.1;    // arrests at a generous radius
  const spread   = (opts.spread || 14) * (opts.scale || 1);      // big outward explosion
  const lift     = opts.lift    != null ? opts.lift    : 1.12;   // mild upward lean (mostly a full ball)
  const trailDt  = opts.trailDt != null ? opts.trailDt : 0.045;  // seconds between trail samples
  // Shape the explosion: spread wider but flatter than the willow.
  const spreadH  = opts.spreadH != null ? opts.spreadH : 1.00;   // 100% horizontal reach (no extra spread)
  const heightK  = opts.heightK != null ? opts.heightK : 0.75;   // -25% explosion height
  const r0       = 0.6;
  ...
  for (let s = 0; s < streams; s++) {
    ...
    v0[s*3]   = Math.cos(theta) * horiz * speed * spreadH;
    v0[s*3+1] = cy * speed * heightK;
    v0[s*3+2] = Math.sin(theta) * horiz * speed * spreadH;
    base[s]   = r0 / speed;
    seed[s]   = Math.random();
  }
  ...
  function setAgeSeconds(t) {
    ...
    // hold the streams lit much longer, then fade quickly over the back 20%
    const lifeFade = norm < 0.8 ? 1 : smooth01((1 - norm) / 0.2);
